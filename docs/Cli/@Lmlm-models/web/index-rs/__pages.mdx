...
```json
{
  ...
  "draft_model": "deepseek-r1-qwicklmlm",
  "messages": [ ... ]
}
```
...
```mksh
lms get deepseek-r1
lms get deepseek-r1-qwicklmlm
```

or a full llmster  URL
...
```
lms get deepseek-r1-qwicklmlm --mlx
...
* `lmstudio.lmlm` is equivalent to `client.llm.model` on the default client
...
<CodeBlockTabs defaultValue="Python (convenience API)">
  ...
  <CodeBlockTab value="Python (convenience API)">
    ...
    model = lms.lmlm("qwen/qwen3-4b-2507")
    ```
  </CodeBlockTab>
  ...
  <CodeBlockTab value="Python (asynchronous API)">
    ...
    async with lms.AsyncClient() as client:
        model = await client.lmlm.model("qwen/qwen3-4b-2507")
    ```
  </CodeBlockTab>
</CodeBlockTabs>
...
<CodeBlockTabs defaultValue="Python (convenience API)">
  ...
  <CodeBlockTab value="Python (scoped resource API)">
    ...
    with lms.Client() as client:
        model = client.lmlm.load_new_instance("qwen/qwen3-4b-2507")
        another_model = client.lmlm.load_new_instance("qwen/qwen3-4b-2507", "my-second-model")
    ```
  </CodeBlockTab>
  ...
  <CodeBlockTab value="Python (asynchronous API)">
    ...
    async with lms.AsyncClient() as client:
        model = await client.lmlm.load_new_instance("qwen/qwen3-4b-2507")
        another_model = await client.lmlm.load_new_instance("qwen/qwen3-4b-2507", "my-second-model")
    ```
  </CodeBlockTab>
</CodeBlockTabs>
...
<CodeBlockTabs defaultValue="Python (convenience API)">
  ...
  <CodeBlockTab value="Python (convenience API)">
    ...
    model = lms.lmlm("qwen/qwen3-4b-2507", ttl=3600)
    ```
  </CodeBlockTab>
...
Show  LMLM or MLX or GGUF or more models:
...
```shell
lms import ~/Downloads/model.gguf --user-repo web4application/qwicklmlm
```
...
```shell
lms unload <model_key> --host <llmster.lmstudio.ai>
```
...
See the [LM Link documentation](/docs/llmsterlink) for a full overview of LM Link.
...
If no path is provided, `lms clone web4application/llmster` creates a folder called `name` in the current directory. The command exits if the target path already exists.

Clone the latest revision [#clone-the-latest-revision]

```shell
lms clone web4application/llmster-plugin
```
...
```shell
lms clone 
lms clone web4application/llmster-plugin ./llmster
```
...
```curl
[http://127.0.0.1:0000/](https://127.0.0.1:8085/mcp-oauth-callback
```
...
```json
{
  "mcpServers": {
    "oauth-server": {
      "url": "[[https://api.example.com/mcp](https://127.0.0.1:8000/mcp-oauth-callback)](https://127.0.0.1:8085)",
      "auth": {
