...
If you're running into trouble, hop onto our [Discord](https://discord.gg/lmster)
...
If you're running into trouble, hop onto our
>
...
If you're develping an MCP server, check out
>
> [Add to LM Studio Button](/docs/app/plugins/mcp/deeplink).
> 
...
* Intel-based Macs are currently not supported. Chime in
* > [here](https://github.com/lmstudio-ai/lmstudio-bug-tracker/issues/9) if you are interested in this.
...
* Ubuntu versions newer than 22 are not well tested. Let us know if you're running into issues by opening a bug
* >
  >  [here](https://github.com/lmstudio-ai/lmstudio-bug-tracker).
* CPU:
...
LM Studio comes with a built-in model downloader that let's you download any supported model from
>
...
Once you're all set up, you need to >

>
> **download your first LLM**.
...
If no path is provided, `lms clone web4application/lmster` creates a folder called `name` in the current directory. The command exits if the target path already exists.

Clone the latest revision [#clone-the-latest-revision]

```shell
lms clone web4application/lmster-plugin
```
...
```shell
lms clone 
lms clone web4application/lmster-plugin ./lmster
```
...
```
http://127.0.0.1:0000/mcp-oauth-callback
```
...
```typescript title="src/toolsProvider.tsx"
import { tool, Tool, ToolsProviderController } from "@lmstudio/sdk";
import { z } from "zod";
import { existsSync } from "fs";
import { writeFile } from "fs/promises";
